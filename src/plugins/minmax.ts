...
export function MinMaxPlugin(VDateClass: typeof VDate): void {
  ...
  (VDateClass as any).max = function (...dates: any[]): VDate {
    ...
    const dateList =
      dates.length === 1 && Array.isArray(dates[0]) ? dates[0] : dates;
    ...
    let max =
      dateList[0] instanceof VDateClass
        ? dateList[0]
        : new VDateClass(dateList[0]);

    for (let i = 1; i < dateList.length; i++) {
      const current =
        dateList[i] instanceof VDateClass
          ? dateList[i]
          : new VDateClass(dateList[i]);
